...
# Quiz Questions
QandA = [
    "Question 1", "Q1 - Option A", "Q1 - Option B", "Q1 - Option C", "Q1 - Option D", "a",
...
def next_question():
    global current
    global label
    if current + 6 < len(QandA):
        current = current + 6
        label.config(text = QandA[current])
    else:
...
def quiz():
    ...
    next_button = Button(window, text = "Next Question", width = 10, command = next_question)
    next_button.place(relx = 0.5, rely = 0.8, relwidth = 0.2, relheight = 0.1, anchor = "n")
