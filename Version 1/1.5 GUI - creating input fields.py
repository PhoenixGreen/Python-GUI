...
window = Tk()
window.title("Simple Input field")
window.geometry('640x640')

# Input / Entry Box
my_text = Entry(window)
my_text.insert(0, "Enter Your Name: ")
my_text.grid(row = 0, column = 0)

# After clicking the button
def after_click():
    my_label = Label(window, text = "Hello " + my_text.get())
    my_label.grid(row = 2, column = 0)

# Create button
my_button = Button(window, text = "Click", command = after_click)
...
'''
Other arguments that can be used for the Entry() function:
